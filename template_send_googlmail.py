...
def generate_html_email_v2(content_title: str, content_detail: str,content_detail_link:str,
    ...
    img_parts = "".join([
        ...
        <div style="margin-bottom:20px;">
          <img src="{img['url']}" alt="PICTURE" style="max-width:600px;" />
          <p style="font-style: italic; color: #555;">{img['caption']}</p>
          <img src="data:image/png;base64,{img['base64str']}" style="width:100%;"/>
        </div>
...
def html_image( pic_url_list, pic_description_list,pic_base64str_list):
    #print('pic_url_list',pic_url_list, pic_description_list,pic_base64str_list)
    image_sections = []
    for p , c, p64 in zip(pic_url_list, pic_description_list,pic_base64str_list):
        temp = { "url": p, 
                "caption":c,
                "base64str":p64
                  }
        image_sections.append(temp)
    #print('image_sections',image_sections)

    return image_sections

from typing import List
def work(df_list, df_titles_list=[], df_description_list=[],pic_url_list=[],pic_description_list=[],pic_base64str_list=[]):
    ...
    image_sections = html_image( pic_url_list, pic_description_list,pic_base64str_list)
    print('image_sections',image_sections)
    ...
    print('len', len(message_html))
    if success:
        print("Python 脚本已将请求提交给 Google Form。请检查 Apps Script 日志和目标邮箱。")
    else:
        print("Python 脚本未能成功将请求提交给 Google Form。", success)
...
if __name__=="__main__":
    ...
    pic_url = "https://drive.google.com/uc?export=view&id=1fIKnJQSI8dOwsNsca7Q9OxUgwq6gsc3H"
    pic_url_list = [pic_url,pic_url]
