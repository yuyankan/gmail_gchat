...
# === 可配置参数 ===
PBI_URL = "https://app.powerbi.com/groups/6bfcff47-831c-42f4-9b91-698f09603fb6/reports/512aa0a2-c2c7-4a17-b86c-b9c2edb9c522/74c669750ae2fb0ddc9b?experience=power-bi"
...
# 构建完整的命令字符串
command_login_chrom = f'"{python_path}" -m playwright codegen {PBI_URL} --save-storage {AUTH_JSON}'
...
def save_new_auth_cookie():
    ...
    os.system(command_login_chrom)# have to close manually, here
        ...
        try:
            ...
            page.wait_for_function(
                """() => {
                    ...
                    if (containers.length < 23) return false;  // 可根据你的图表数量调整
                    return Array.from(containers).every(container => {
                        const visible = container.offsetWidth > 10 && container.offsetHeight > 10;
                        const hasChart = container.querySelector('svg, canvas, .bar, .card, .textRun,table,.pivotTable,p') !== null;
